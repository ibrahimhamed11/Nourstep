...
const content: I18n<CountdownContent> = {
  en: {
    ...
    subtitle: 'Be among the first to experience NourStep. Join our early access list.',
  ...
  ar: {
    ...
    subtitle: 'كن من أوائل المستخدمين لمنصة نور ستيب. سجّل في قائمة الوصول المبكر.',
...
export default function Countdown({ lang }: { lang: Lang }) {
  ...
  return (
    <section id="countdown" className="relative py-24 md:py-32 px-4 bg-darkblue">
      ...
      {/* Glow effects */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-royal/10 rounded-full blur-3xl" />
      ...
      <div className="max-w-4xl mx-auto relative">
        ...
        >
          ...
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-heading font-extrabold text-white mb-2">
            {t.title}{' '}
        ...
        >
          {timeUnits.map((unit, i) => (
            <div key={i} className="card-dark p-6 md:p-8 text-center">
              <div className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-white tabular-nums font-heading leading-none">
                {String(unit.value).padStart(2, '0')}
              </div>
              <div className="text-xs md:text-sm text-muted mt-2 font-semibold uppercase tracking-widest">
                {unit.label}
              </div>
            </div>
          ))}
        ...
        >
          ...
          <a
            href="#"
            className="inline-flex items-center gap-2.5 px-8 py-4 rounded-2xl font-semibold text-white bg-royal hover:bg-bright transition-all duration-300 shadow-lg shadow-royal/30 hover:shadow-bright/30 hover:-translate-y-0.5"
          >
            <Rocket size={18} />
            {t.cta}
          </a>
